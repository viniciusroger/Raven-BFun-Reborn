...
public class GeneralUtils {
    ...
    public static boolean addEnemy(String name) {
        if (enemies.add(name.toLowerCase())) {
            GeneralUtils.sendMessage("&7Added &cenemy&7: &b" + name);
            return true;
        }
        return false;
    }
    ...
    public static boolean removeEnemy(String name) {
        if (enemies.remove(name.toLowerCase())) {
            GeneralUtils.sendMessage("&Removed &cenemy&7: &b" + name);
            return true;
        }
        return false;
    }
    ...
    public static boolean isClicking() {
        if (ModuleManager.leftClicker.isEnabled()) {
            ...
            return CPSUtil.f() > 1 && System.currentTimeMillis() - CPSUtil.LL < 300L;
        }
    }
