...
public class RenderUtils {
    ...
    public static void renderBPS(final boolean b, final boolean b2) {
        ...
        int n = -1;
        if (b) {
            final double t = GeneralUtils.gbps((Freecam.freeEntity == null) ? mc.thePlayer : Freecam.freeEntity, 2);
            if (t < 10.0) {
        ...
        if (b2) {
            final double h = GeneralUtils.getHorizontalSpeed();
            if (!s.isEmpty()) {
                s += " ";
            }
            s += GeneralUtils.rnd(h, 3);
        }
        ...
        if (e instanceof EntityLivingBase) {
            ...
            if (type == 3) {
                ...
                if (color != 0) {
                    ...
                    int st = GeneralUtils.getChroma(2L, 0L);
                    int en = GeneralUtils.getChroma(2L, 1000L);
    ...
    public static void dtl(Entity e, int color, float lw) {
        if (e != null) {
            double x = e.lastTickPosX + (e.posX - e.lastTickPosX) * (double) GeneralUtils.getTimer().renderPartialTicks - mc.getRenderManager().viewerPosX;
            double y = (double) e.getEyeHeight() + e.lastTickPosY + (e.posY - e.lastTickPosY) * (double) GeneralUtils.getTimer().renderPartialTicks - mc.getRenderManager().viewerPosY;
            double z = e.lastTickPosZ + (e.posZ - e.lastTickPosZ) * (double) GeneralUtils.getTimer().renderPartialTicks - mc.getRenderManager().viewerPosZ;
            float a = (float) (color >> 24 & 255) / 255.0F;
            float r = (float) (color >> 16 & 255) / 255.0F;
            float g = (float) (color >> 8 & 255) / 255.0F;
    ...
    public static void dct(String text, char lineSplit, int x, int y, long s, long shift, boolean rect, FontRenderer fontRenderer) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (c == lineSplit) {
                ...
                fontRenderer.drawString(String.valueOf(c), (float) x, (float) y, GeneralUtils.getChroma(s, r), rect);
    ...
    public static void d3p(double x, double y, double z, double radius, int sides, float lineWidth, int color, boolean chroma) {
        ...
        for (int i = 0; i < sides * 2; ++i) {
            if (chroma) {
                ...
                int c = GeneralUtils.getChroma(2L, d);
